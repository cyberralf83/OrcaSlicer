...
Line 1 above is upstream's entire CLAUDE.md - keep it first so upstream's own edits to this file merge
instead of conflicting (it has conflicted twice: merges 62d7904b39 and 835c044c67). Fork content goes
below it.
...
- **A clean merge at an override site is a hazard signal, not a success.** The further a fork override sits from the upstream statement it overrides, the more silently it survives upstream rewriting, deleting, or gating that statement. Live example: upstream's `8dfc7a14b9`/`32f82b64e7` changed the BBL default-button rule to `is_bbl_vendor() || use_printer_agents`; both merge clean and the fork's override discards the new rule for BBL with no signal. Prefer a pinning test over a comment where the divergence would matter.
- **Measuring upstream churn: `git log -L`, `git log -- <file>` and `git blame` are UNRELIABLE in this repo without `--full-history`.** Upstream carries a parentless root commit `3bee58fcab` (2026-07-06) that "adds" all 4.3M lines of every file, which terminates default history simplification. `git log --oneline upstream-nightly -- src/slic3r/GUI/GLToolbar.cpp` returns **1** commit; with `--full-history` it returns **485**. Always pass `--full-history`, and prefer `-G'<pattern>' --full-history` over line-range logs. An earlier churn table built without this was entirely artifact and led to the wrong files being restructured.
- **What has actually caused conflicts** (census of 96 upstream merges: 86 clean, 10 conflicted): merge *topology* (one event, 88 files, caused by committing partial/unresolved merges — never do that, `git merge --abort` instead); upstream-owned meta files the fork rewrote (6 events: `CLAUDE.md`, `AGENTS.md`, `.gitignore`); fork tests inside upstream test files (3 events, fixed); fork insertions *adjacent* to upstream edits (4 events, invisible to any deletion-count metric); in-place line rewrites (2 events). Prioritise accordingly — deletion count is a poor proxy for conflict risk.
