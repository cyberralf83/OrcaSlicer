...
void ConfigManipulation::update_print_fff_config(DynamicPrintConfig* config, const bool is_global_config, const bool is_plate_config)
{
    ...
    // Beam density control thins the interlocking beams (keep N beams, skip a gap,
    // repeat). Both 'Beam group count' and 'Beam gap' must be > 0 for it to engage —
    // the density branch in InterlockingGenerator is gated on `group > 0 && gap > 0`.
    ...
    // The latch is process-wide (see ConfigManipulation.hpp): the per-object paths construct a fresh
    // ConfigManipulation on every settings change, so an instance flag never suppressed the repeat.
    // Still warn on the object paths - per-object beam overrides are real - but not for plate config,
    // which has no interlocking settings of its own.
    bool beam_interlocking_on = !is_plate_config && config->opt_bool("interlocking_beam");
...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, int variant_index, const bool is_global_config)
{
    ...
    for (auto el : { "extra_perimeters_on_overhangs", "ensure_vertical_shell_thickness", "detect_thin_wall", "detect_overhang_wall",
        "seam_position", "staggered_inner_seams", "seam_hide_at_interface", "wall_sequence", "outer_wall_line_width" })
...
//    toggle_line("support_interface_speed", have_support_material && have_support_interface);
    ...
    bool hide_seam_at_interface = have_perimeters && config->opt_bool("seam_hide_at_interface");
    toggle_line("seam_interface_depth", hide_seam_at_interface);
    toggle_line("seam_interface_skip_bottom_layers", hide_seam_at_interface);
