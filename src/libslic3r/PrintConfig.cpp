...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    def->sidetext = L("mm");
    // Deliberately allows 0; the usable floor is applied in seam_point_is_embedded_enough instead.
    // Tightening def->min here would make an existing preset that stores 0 fail validate(), which
    // aborts the CLI (CLI_INVALID_VALUES_IN_3MF) and silently blanks the field in the GUI.
    def->min = 0;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloat(2.0));
